...
IMPLEMENT_DYNAMIC(CMultiSelTree, CTreeCtrl)
...
UINT CMultiSelTree::GetSelectedCount() const
{
	UINT nCount = 0;
...
BOOL CMultiSelTree::SetMultiSelect(BOOL bMulti)
{
	BOOL b = m_bMulti;
	m_bMulti = bMulti;
	if (!m_bMulti) {
...
BOOL CMultiSelTree::SetItemState(HTREEITEM hItem, UINT nState, UINT nStateMask)
{
	ASSERT(hItem);
...
UINT CMultiSelTree::GetItemState(HTREEITEM hItem, UINT nStateMask) const
{
...
BOOL CMultiSelTree::_SendNotify(LPNMHDR pNMHDR)
{
	ASSERT(::GetParent(m_hWnd));	//never expected this

	BOOL b = m_bEmulated;
	m_bEmulated = TRUE;
	BOOL bRes = ::SendMessage(::GetParent(m_hWnd), WM_NOTIFY, (WPARAM)pNMHDR->idFrom, (LPARAM)pNMHDR) != 0;
	m_bEmulated = b;
	return bRes;
}
...
HTREEITEM CMultiSelTree::GetFirstSelectedItem() const
{
...
HTREEITEM CMultiSelTree::GetNextSelectedItem(HTREEITEM hItem) const
{
	hItem = GetNextVisibleItem(hItem);
	while (hItem) {
		if (IsSelected(hItem))
...
void CMultiSelTree::SelectAll(BOOL bSelect /*= TRUE*/)
{
...
void CMultiSelTree::SelectAllIgnore(BOOL bSelect, HTREEITEM hIgnore)
{
...
void CMultiSelTree::SelectRange(HTREEITEM hFirst, HTREEITEM hLast, BOOL bOnly /*= TRUE*/)
{
...
void CMultiSelTree::OnButtonDown(BOOL bLeft, UINT nFlags, CPoint point)
{
	UINT nHF;
	HTREEITEM hItem;
	...
	const bool ENABLE_FULLROW_SEL = 1;

	DWORD htRubber = TVHT_NOWHERE|TVHT_ONITEMINDENT;
	if( ! ENABLE_FULLROW_SEL )
		htRubber |= TVHT_ONITEMRIGHT;

	if (!hItem || (nHF & htRubber ) ) {
...
//		ENABLE_FULLROW_SEL ? TVHT_ONITEMRIGHT : 0 ) );	//nothing else left

	DoPreSelection(hItem, bLeft, nFlags);
	DoAction(hItem, bLeft, nFlags, point);
}

void CMultiSelTree::DoPreSelection(HTREEITEM hItem, BOOL bLeft, UINT nFlags)
{
	if (bLeft) {
...
void CMultiSelTree::DoAction(HTREEITEM hItem, BOOL bLeft, UINT nFlags, CPoint point)
{
	::SetCapture(m_hWnd);
...
void CMultiSelTree::DoBanding(BOOL bLeft, UINT nFlags, CPoint point)
{
	if (::GetFocus() != m_hWnd)
		::SetFocus(m_hWnd);
...
void CMultiSelTree::UpdateSelectionForRect(LPCRECT pRect, UINT nFlags, CTreeItemList& list)
{
	CRect rect;
	BOOL bSel;
	POSITION pos;
...
void CMultiSelTree::OnSetFocus(CWnd* pOldWnd) 
{
...
void CMultiSelTree::OnKillFocus(CWnd* pNewWnd) 
{
...
BOOL CMultiSelTree::OnItemExpanding(NMHDR* pNMHDR, LRESULT* pResult) 
{
	if (!m_bMulti)
		return FALSE;
...
BOOL CMultiSelTree::SelectChildren(HTREEITEM hParent, BOOL bSelect /*= TRUE*/, BOOL bRecurse /*= TRUE*/)
{
...
void CMultiSelTree::GetSelectedList(CTreeItemList& list) const
{
	list.RemoveAll();
