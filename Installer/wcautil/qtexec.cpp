    ...
    )
{
    ...
    if (!::CreatePipe(&hOutTemp, &hOutWrite, &sa, 0))
        ExitOnLastError(hr, L"failed to create output pipe");

    if (!::CreatePipe(&hInRead, &hInTemp, &sa, 0))
        ExitOnLastError(hr, L"failed to create input pipe");
    ...
    if (!::DuplicateHandle(::GetCurrentProcess(), hOutWrite, ::GetCurrentProcess(), &hErrWrite, 0, TRUE, DUPLICATE_SAME_ACCESS))
        ExitOnLastError(hr, L"failed to duplicate write handle");
    ...
    if (!::DuplicateHandle(::GetCurrentProcess(), hOutTemp, ::GetCurrentProcess(), &hOutRead, 0, FALSE, DUPLICATE_SAME_ACCESS))
        ExitOnLastError(hr, L"failed to duplicate output pipe");

    if (!::DuplicateHandle(::GetCurrentProcess(), hInTemp, ::GetCurrentProcess(), &hInWrite, 0, FALSE, DUPLICATE_SAME_ACCESS))
        ExitOnLastError(hr, L"failed to duplicate input pipe");
    ...
    )
{
    ...
    ExitOnNull(pBuffer, hr, E_OUTOFMEMORY, L"failed to allocate buffer for output.");
    ...
    while (0 != dwBytes)
    {
        ...
        {
            ExitOnLastError(hr, L"failed to read from handle.");
        }
        ...
        if (bUnicode)
        {
            ...
            ExitOnFailure(hr, L"failed to concatenate output strings");
        }
        else
        {
            hr = StrAllocStringAnsi(&szTemp, (LPSTR)pBuffer, 0, CP_OEMCP);
            ExitOnFailure(hr, L"failed to allocate output string");
        ...
        while (pEnd && *pEnd)
        {
            ...
            // Log output
            hr = StrAllocString(&szWrite, pNext, 0 );
            ExitOnFailure(hr, L"failed to allocate string");
    ...
    if (szLog && *szLog)
    {
        hr = StrAllocString(&szWrite, szLog, 0);
        ExitOnFailure(hr, L"failed to allocate string");
    ...
    )
{
    ...
    ExitOnFailure(hr, L"failed to create output pipes");
    ...
    WcaLog(LOGMSG_VERBOSE, L"%s", wzCommand);
        ...
        &oProcInfo))
    {
        ...
        ExitOnLastError(hr, L"Command failed to execute.");

    hr = HRESULT_FROM_WIN32(dwExitCode);
    ExitOnFailure(hr, L"Command line returned an error.");
