    ...
    )
{
    ...
    {
        ExitOnFailure1(hr = E_OUTOFMEMORY, L"Not enough memory to allocate string of size: %d", cch);
    }
    ...
    ExitOnNull1(pwz, hr, E_OUTOFMEMORY, L"failed to allocate string, len: %d", cch);
    ...
    )
{
    ...
    ExitOnNull1(psz, hr, E_OUTOFMEMORY, L"failed to allocate string, len: %d", cch);
    ...
    )
{
    ...
    if (*ppwz)
    {
        ...
        if (-1 == cch)
            ExitOnFailure(hr = E_INVALIDARG, L"failed to get size of destination string");
    ...
    if (cch < cchSource + 1)
    {
        ...
        ExitOnFailure1(hr, L"failed to allocate string from string: %s", wzSource);
    }
    ...
    )
{
    ...
    if (0 == cchSource)
    {
        ...
        if (0 == cchDest)
        {
            ExitWithLastError1(hr, L"failed to get required size for conversion to ANSI: %s", wzSource);
        }
    ...
    {
        ExitWithLastError1(hr, L"failed to convert to ansi: %s", wzSource);
    }
    (*ppsz)[cchDest] = L'\0';
    ...
    )
{
    ...
    if (0 == cchSource)
    {
        ...
        if (0 == cchDest)
        {
            ExitWithLastError1(hr, L"failed to get required size for conversion to unicode: %s", szSource);
        }
    ...
    {
        ExitWithLastError1(hr, L"failed to convert to unicode: %s", szSource);
    }
    (*ppwz)[cchDest] = L'\0';
    ...
    )
{
    ...
    if (cch - cchLen < cchPrefix + 1)
    {
        ...
        hr = StrAlloc(ppwz, cch);
        ExitOnFailure1(hr, L"failed to allocate string from string: %s", wzPrefix);
    }
    ...
    {
        ExitOnFailure(hr = E_UNEXPECTED, L"for some reason our buffer is still null");
    }
    ...
    )
{
    ...
    if (0 == cch)   // if there is no space in the string buffer
    {
        cch = 256;
        hr = StrAlloc(ppwz, cch);
        ExitOnFailure1(hr, L"failed to allocate string to format: %s", wzFormat);
    }
    ...
    )
{
    ...
    if (0 == cch)   // if there is no space in the string buffer
    {
        ...
        ExitOnFailure1(hr, L"failed to allocate string to format: %s", szFormat);
    }
    ...
    )
{
    ...
    ExitOnFailure(hr, L"failed to free string");
LExit:
    return hr;
}
    ...
    )
{
    ...
    if (cbDest < cchSource / 2)
    {
        ...
        ExitOnFailure3(hr, L"Insufficient buffer to decode string '%s' len: %d into %d bytes.", wzSource, cchSource, cbDest);
    }
    ...
    )
{
    ...
    ExitOnFailure(hr, L"failed to allocate destination string");
    ...
    )
{
    ...
    *ppbDest = static_cast<BYTE*>(MemAlloc(cbDest, FALSE));
    ExitOnNull(*ppbDest, hr, E_OUTOFMEMORY, L"failed allocate memory to decode the string");
    ...
    )
{
    ...
    ExitOnFailure(hr, L"failed to get the max size of a string while calculating MULTISZ length");
    ...
    )
{
    ...
    else
    {
        ...
        ExitOnFailure(hr, L"failed to get length of multisz");
    }
    ...
    ExitOnFailure(hr, L"failed to allocate result string");

    // Prepend
    hr = StringCchCopyW(pwzResult, cchResult, pwzInsert);
    ExitOnFailure1(hr, L"failed to copy prepend string: %s", pwzInsert);
    ...
    )
{
    ...
    ExitOnFailure(hr, L"failed to get the length of a MULTISZ string");
    ...
    )
{
    ...
    if (S_OK == hr)
    {
        ...
        if (cchProgress > cchMultiSz)
            ExitOnFailure(hr = E_UNEXPECTED, L"failed to move past the string to be removed from MULTISZ");
    ...
    )
{
    ...
    while (dwCurrentIndex < dwIndex)
    {
        ...
        if ((dwCurrentIndex + 1 != dwIndex && L'\0' == *(wz + 1)) || cchProgress >= cchMultiSz)
            ExitOnFailure1(hr = HRESULT_FROM_WIN32(ERROR_OBJECT_NOT_FOUND), L"requested to insert into an invalid index: %d in a MULTISZ", dwIndex);
    ...
    hr = StrAlloc(&pwzResult, cchResult);
    ExitOnFailure(hr, L"failed to allocate result string for MULTISZ insert");
    ...
    )
{
    ...
    ExitOnFailure1(hr, L"failed to remove string from MULTISZ at the specified index: %d", dwIndex);

    hr = MultiSzInsertString(ppwzMultiSz, NULL, dwIndex, pwzString);
    ExitOnFailure1(hr, L"failed to insert string into MULTISZ at the specified index: %d", dwIndex);
