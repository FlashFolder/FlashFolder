...
public:
	CFolderDlg(	IN LPCTSTR	lpszTitle	= NULL, 
					IN LPCTSTR	lpszSelPath	= NULL,
					IN CWnd*	pParentWnd	= NULL,
					IN UINT		uFlags		= BIF_RETURNONLYFSDIRS | BIF_USENEWUI );
	virtual ~CFolderDlg( void );
...
AFX_INLINE BOOL CFolderDlg::SetSelectedFolder( IN LPCTSTR lpszPath )
	{ ASSERT ( lpszPath != NULL ); 
		return( ::lstrcpy( m_szSelPath, lpszPath ) != NULL ); }

AFX_INLINE LPCTSTR CFolderDlg::GetSelectedFolder( void ) const
	{ return m_szSelPath; }

AFX_INLINE BROWSEINFO& CFolderDlg::GetBI( void )
	{ return m_bi; }

AFX_INLINE const BROWSEINFO& CFolderDlg::GetBI( void ) const
	{ return m_bi; }

/////////////////////////////////////////////////////////////////////////////
// Filled after a call to DoModal

AFX_INLINE LPCTSTR CFolderDlg::GetFolderPath( void ) const
	{ return m_szFolPath; }

AFX_INLINE LPCTSTR CFolderDlg::GetFolderName( void ) const
	{ return m_bi.pszDisplayName; }

AFX_INLINE INT CFolderDlg::GetFolderImage( void ) const
	{ return m_bi.iImage; }

/////////////////////////////////////////////////////////////////////////////
// Commands, valid to call only from handlers

AFX_INLINE void CFolderDlg::EnableOK( IN BOOL bEnable /*TRUE*/ )
	{ ASSERT( m_hWnd != NULL ); 
		::SendMessage( m_hWnd, BFFM_ENABLEOK, (WPARAM)bEnable, 0L );}

AFX_INLINE void CFolderDlg::SetSelection( IN LPITEMIDLIST lpItemIDList )
	{ ASSERT( m_hWnd != NULL ); 
		::SendMessage( m_hWnd, BFFM_SETSELECTION, (WPARAM)FALSE, (LPARAM)lpItemIDList ); }

AFX_INLINE void CFolderDlg::SetSelection( IN LPCTSTR lpszFolderPath )
	{ ASSERT( m_hWnd != NULL );
		::SendMessage( m_hWnd, BFFM_SETSELECTION, (WPARAM)TRUE, (LPARAM)lpszFolderPath ); }

AFX_INLINE void CFolderDlg::SetStatusText( IN LPCTSTR lpszText )
	{ ASSERT( m_hWnd != NULL );
		::SendMessage( m_hWnd, BFFM_SETSTATUSTEXT, (WPARAM)0, (LPARAM)lpszText ); }

// Shell version 5.0 or later:

AFX_INLINE void CFolderDlg::SetExpanded( IN LPITEMIDLIST lpItemIDList )
	{ ASSERT( m_hWnd != NULL ); 
